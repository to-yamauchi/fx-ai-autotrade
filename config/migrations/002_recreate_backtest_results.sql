...
-- ================================
--
-- ファイル名: 002_recreate_backtest_results.sql
-- パス: config/migrations/002_recreate_backtest_results.sql
--
-- 【概要】
-- 既存のbacktest_resultsテーブルを削除して、正しいスキーマで再作成します。
--
-- 【実行方法】
-- psql -U postgres -d fx_autotrade -f config/migrations/002_recreate_backtest_results.sql
--
-- 【作成日】2025-10-23
...
CREATE INDEX idx_backtest_model ON backtest_results(ai_model);
...
-- 完了メッセージ
SELECT 'backtest_results テーブルを再作成しました！' AS message;
